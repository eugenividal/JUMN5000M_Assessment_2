...
The result is a data frame with all the features that contains 423325 observations. Why there are 1,755 more observations?
...
```{r}
# join store-level data onto training dataset (so we know size and type).
train_joined1 = left_join (train, y = stores)

# join train_joined1 onto features dataset (so we know the rest of variables). 
train_joined2 = left_join (train_joined1, y = features)
