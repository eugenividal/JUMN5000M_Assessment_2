...
5. Finally, the most important variable (dependent variable) is ‘Weekly Sales‘ isn’t distributed normally at all. There’s a whole pile of very small values at one end, and the larger values aren’t visible at all.
...
Except temperature and employment, the rest of variables aren't normal distributed either.

```{r}
hist(train_joined$Size)
hist(train_joined$Temperature)
hist(train_joined$Fuel_Price)
hist(train_joined$CPI)
hist(train_joined$Unemployment)
```
...
Some variables such as `Type` and `Size` give very similar information (pressumably, size is based on type), although `Size` is much more precise and numerial. Consequeltly, We will remove the `Type` variable.

```{r}
# remove promotion columns
train_joined$Type <- NULL
```
head(train_joined)
...
### Transforming the sales variable

The data is not normal, we will take the log and not looks more normal distributed.

In order to make the variable better fit the assumptions underlying regression (the model we will apply), we need to transform it. The most common way to do this is to take the log of ‘Weekly Sales’. 

This is done using a derived variable and the ‘log’ statement; to take the log of ‘Weekly Sales’ plus a nominal value of 1, because I can’t take a log of zero.

When we call up a histogram of ‘Log of Weekly Sales', we can see that the distribution is significantly closer to the normal probability distribution. It’s a bit skewed to one side, but a big improvement over the untransformed variable.

The goal in transforming the variables is to make the relationship between them more linear. 

Linear relationships that are not evident using an untransformed form  may be rendered detectable after transformation. So, in short, we transform variables in hopes of improving the overall model, which after all is a linear model.
...
fit1<- lm(log(train_joined.train$Weekly_Sales) ~ log(Size), data = train_joined.train)
summary(fit1)
library(ggplot2)
ggplot(train_joined.train, aes(x=log(Size), y=log(Weekly_Sales))) + geom_point() + geom_smooth(method='lm') + labs(title = "weekly sales vs Size", x = "Size", y = "Sales", color = "Type of store")
...
```{r, echo=FALSE}
fit2 <- lm(log(train_joined.train$Weekly_Sales) ~ log(Temperature), data = train_joined.train)
summary(fit2)
ggplot(train_joined.train, aes(x=log(Temperature), y=log(Weekly_Sales))) + geom_point() + geom_smooth(method='lm') + labs(title = "weekly sales vs Temperature", x = "Temperature", y = "Sales", color = "Type of store")
```

```{r, echo=FALSE}
fit3 <- lm(Weekly_Sales ~ Fuel_Price, data = train_joined.train)
summary(fit3)
ggplot(train_joined.train, aes(x=Fuel_Price, y=Weekly_Sales)) + geom_point() + geom_smooth(method='lm') + labs(title = "weekly sales vs Fuel Price", x = "Fuel Price", y = "Sales", color = "Type of store")
```

```{r, echo=FALSE}
fit4 <- lm(Weekly_Sales ~ CPI, data = train_joined.train)
summary(fit4)
ggplot(train_joined.train, aes(x=Fuel_Price, y=Weekly_Sales)) + geom_point() + geom_smooth(method='lm') + labs(title = "weekly sales vs CPI", x = "CPI", y = "Sales", color = "Type of store")
```

```{r, echo=FALSE}
fit5 <- lm(Weekly_Sales ~ Unemployment, data = train_joined.train)
summary(fit5)
ggplot(train_joined.train, aes(x=Unemployment, y=Weekly_Sales))+ geom_point() + geom_smooth(method='lm') + labs(title = "weekly sales vs Unemployment", x = "Unemployment", y = "Sales", color = "Type of store")
```
...
### Simple regression among independent variables (coliniarity analysis)

```{r, echo=FALSE}
fit5 <- lm(CPI ~ Fuel_Price, data = train_joined.train)
summary(fit5)
ggplot(train_joined.train, aes(x=Fuel_Price, y=CPI))+ geom_point() + geom_smooth(method='lm') + labs(title = "CPI vs Fuel Price", x = "Fuel Price", y = "CPI")
```

```{r, echo=FALSE}
fit5 <- lm(CPI ~ Unemployment, data = train_joined.train)
summary(fit5)
ggplot(train_joined.train, aes(x=Unemployment, y=CPI))+ geom_point() + geom_smooth(method='lm') + labs(title = "CPI vs Unemployment", x = "Unemployment", y = "CPI")
```

### Computing the correlation matrix

```{r}
numericdataset <- train_joined.train[,-c(1,2,3,5),drop=FALSE]
...
```{r}
fit <- lm(Weekly_Sales ~ Size + CPI + Unemployment, data= train_joined.train)
summary(fit)
```
